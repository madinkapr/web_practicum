...
// API dan cardlarni olib DOM yaratish
async function renderCards() {
  ...
  container.innerHTML = `
    <div class="card-column column-0"></div>
    <div class="card-column column-1"></div>
  `;

  const col0 = container.querySelector(".column-0");
  const col1 = container.querySelector(".column-1");

  data.forEach((card, index) => {

    const cardEl = document.createElement("div");
    cardEl.className = `card card-color-${index}`;

    cardEl.innerHTML = `
      <div class="border"></div>
      <img src="http://localhost:3000${card.coverImg}" alt="card-${card.id}"/>
      <h1>${card.title}</h1>
    `;

    if (index % 2 === 0) {
      col0.appendChild(cardEl);
    } else {
